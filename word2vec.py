...
valid_examples = np.array(random.sample(range(valid_window), valid_size)) # pick 16 elements from 0-100
...
with graph.as_default(), tf.device('/cpu:0'):
  ...
  train_labels = tf.placeholder(tf.int32, shape=[batch_size, 1]) # batch_size * num of target classes
...
print(two_d_embeddings.shape[0])
...
def generate_batch(batch_size, bag_window):
    ...
    for _ in range(span):
        buffer.append(data[data_index])
        data_index = (data_index + 1) % len(data)
    for i in range(batch_size):#loop 4 times
        buffer_list = list(buffer)
        labels[i,0] = buffer_list.pop(bag_window)#pop 3081
        batch[i] = buffer_list # 5234, 12
        buffer.append(data[data_index])#3081, 12, 6
        data_index = (data_index + 1) % len(data)
    return batch,labels
