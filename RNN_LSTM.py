...
def maybe_download(filename, expected_size): #download file, need specify which file under the url and its size
    if not os.path.exists(filename): # need to verify whether the file is already existed 
        filename, _ = urlretrieve(url+filename, filename) # give the download path, and specify where to save the file 
...
#   dbg.set_trace()
  ...
  representation."""
  """For example: 11 batches, each batch is 64*27 array"""
  s = [''] * batches[0].shape[0]
  for b in batches:
...
with graph.as_default():
  ...
  def lstm_cell(i, o, state):
    ...
    input_gate = tf.sigmoid(tf.matmul(i, ix) + tf.matmul(o, im) + ib) #i(t) = sigma(W(i).[h(t-1), x(t)] + b(i)
    forget_gate = tf.sigmoid(tf.matmul(i, fx) + tf.matmul(o, fm) + fb)#f(t) = sigma(W(f).[h(t-1), x(t)] + b(f)
    update = tf.matmul(i, cx) + tf.matmul(o, cm) + cb #c(t) = tanh(W(c).[h(t-1), x(t)] + b(c)
    state = forget_gate * state + input_gate * tf.tanh(update) # c(t) = f(t). c(t-1) + i(t).c(t)
    output_gate = tf.sigmoid(tf.matmul(i, ox) + tf.matmul(o, om) + ob) # o(t) = sigma(W(o).[h(t-1), x(t)] + b(o))
    return output_gate * tf.tanh(state), state  # h(t) = o(t) * tanh(c(t))
    # return current LSTM output and current state--Ct--
...
with graph.as_default():
  ...
  xx = tf.concat([ix, fx, cx, ox], 1)
  mm = tf.concat([im, fm, cm, om], 1)
  bb = tf.concat([ib, fb, cb, ob], 1)
  # Variables saving state across unrollings.
  saved_output = tf.Variable(tf.zeros([batch_size, num_nodes]), trainable=False)
  saved_state = tf.Variable(tf.zeros([batch_size, num_nodes]), trainable=False)
  # Classifier weights and biases.
  ...
  def lstm_cell(i, o, state):
    ...
    Note that in this formulation, we omit the various connections between the
    previous state and the gates."""
    matmuls = tf.sigmoid(tf.matmul(i, xx) + tf.matmul(o, mm) + bb)
    matmuls_input, matmuls_forget, update, matmuls_output = tf.split(matmuls, num_or_size_splits = 4, axis = 1)
    input_gate = tf.sigmoid(matmuls_input) #i(t) = sigma(W(i).[h(t-1), x(t)] + b(i)
    forget_gate = tf.sigmoid(matmuls_forget)#f(t) = sigma(W(f).[h(t-1), x(t)] + b(f)
#     update = tf.matmul(i, cx) + tf.matmul(o, cm) + cb #c(t) = tanh(W(c).[h(t-1), x(t)] + b(c)
    state = forget_gate * state + input_gate * tf.tanh(update) # c(t) = f(t). c(t-1) + i(t).c(t)
    output_gate = tf.sigmoid(matmuls_output) # o(t) = sigma(W(o).[h(t-1), x(t)] + b(o))
    return output_gate * tf.tanh(state), state  # h(t) = o(t) * tanh(c(t))
    # return current LSTM output and current state--Ct--
